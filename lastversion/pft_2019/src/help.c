...
void		help_debug(void)
{
	...
	help_paragraph("PFT must run in debugger compatibility mode to work with debuggers. You must also include the -g flag in your own 1Makefile. The following works with your current configuration:");
	printf("\n");
	if (SINGLE_NUMBER_SINGLE_TEST && SINGLE_TEST_TURNS_ON_LLDB_COMPAT_MODE)
	{
...
void		help_how_it_works(void)
{
	...
	help_paragraph("The 1Makefile creates two copies of unit_tests.c, one that uses ft_printf, and one that uses printf. For each test, it calls both functions and tests their return value and output.");
}
...
void		help_remove_history_when_tests_new(void)
{
	...
	help_paragraph("This option in `options-config.ini` determines whether the 1Makefile will remove the history log whenever unit_tests.c is strictly newer. There are also options in the enable-test and disable-test scripts that can prevent the removal trigger when these scripts are used.");
	printf("\n");
	help_header("See Also");
	help_example("./test help history", 0);
}
